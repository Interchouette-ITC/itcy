#!/usr/bin/env bash
# X/Twitter discovery via warm Brave profile (pw/profile-x). Not corpus storage.
#
# Gold profile (login vault) is never opened by Playwright. Each run copies it to
# pw/profile-x-run/… so a login-wall / guest rewrite cannot poison the vault.
#
# Scrapes attach to a real Brave via CDP (not launchPersistentContext), which
# keeps the encrypted cookie jar usable and reduces automation signals.
#
# Usage:
#   scripts/fetch-twitter-pulse.sh following
#   scripts/fetch-twitter-pulse.sh search '<query>' ['<query2>' ...]
#   scripts/fetch-twitter-pulse.sh digest '<query>' ['<query2>' ...]
# Prints JSON array of {title,url,subject,detail,lane} to stdout.
# digest = home timeline then searches on the same Brave page (one window).
# Search queries are forced to English (lang:en) inside the Node scraper.
set -euo pipefail
MODE="${1:?mode required: following|search|digest}"
shift || true
...
cd "${ROOT}"
# Pass mode + remaining args (search queries) through to Node.
env PLAYWRIGHT_REQUIRE_FROM="${PW_JSON}" ITCY_TWITTER_CDP_URL="http://127.0.0.1:${CDP_PORT}" \
  node "${ROOT}/scripts/fetch-twitter-pulse.mjs" \
  "${MODE}" "$@"
